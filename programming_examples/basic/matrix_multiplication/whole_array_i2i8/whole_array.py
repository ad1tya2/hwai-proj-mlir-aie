...
def main():
    ...
    argparser.add_argument(
        "--dtype_in", type=str, choices=["bf16", "i8", "i16", "i2"], default="i16"
    )
    argparser.add_argument(
        "--dtype_out",
        type=str,
...
def my_matmul(
    ...
    # Handle i2: physically it's int8, but we need to adjust sizes.
    is_i2 = (dtype_in_str == "i2")
    physical_dtype_in_str = "i8" if is_i2 else dtype_in_str
    
    dtype_in = str_to_dtype(physical_dtype_in_str)
    ...
    @device(dev_ty)
    def device_body():
        # For i2, A is packed. Size is m * k / 4 bytes.
        # But np.ndarray type should reflect physical type (int8) and physical size.
        # m * k elements of i2 -> m * k / 4 elements of int8.
        
        A_size_factor = 4 if is_i2 else 1
        
        A_l2_ty = np.ndarray[(m * k * n_A_tiles_per_shim // A_size_factor,), np.dtype[dtype_in]]
        B_l2_ty = np.ndarray[(k * n,), np.dtype[dtype_in]]
        C_l2_ty = np.ndarray[(m * n * n_aie_rows,), np.dtype[dtype_out]]
        A_l1_ty = np.ndarray[(m, k // A_size_factor), np.dtype[dtype_in]]
        ...
        for row in range(n_aie_rows):
            A_l2l1_fifos[row] = object_fifo(
                ...
                (
                    [
                        (m // r, r * k // A_size_factor),
                        ((k // A_size_factor) // (s // A_size_factor), s // A_size_factor),
                        (r, k // A_size_factor),
                        (s // A_size_factor, 1),
                    ]
                    if not use_scalar
        ...
        for i in range(n_shim_mem_A):
            ...
            if stop_row - start_row > 1:
                of_offsets = [m * k // A_size_factor * j for j in range(stop_row - start_row)]
            else:
                of_offsets = []
            object_fifo_link(
                A_l3l2_fifos[i],
        ...
        @runtime_sequence(
            np.ndarray[(M * K // A_size_factor,), np.dtype[dtype_in]],
                ...
                for pingpong in [0, 1]:
                    ...
                    for col in range(n_aie_cols):
                        ...
                        for tile_row in range(tb_n_rows):
                            ...
                            #      ----------------
                            A_block_offset = (
                                (row_base + tile_row) * n_aie_rows * m * K // A_size_factor
                            )  # base address for this transfer block for all BDs
                            A_row_offset = (
                                col * n_A_tiles_per_shim * m * K // A_size_factor
                            ...
                            A_strides = [0, k // A_size_factor, K // A_size_factor, 1]
