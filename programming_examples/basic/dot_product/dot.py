...
def my_dot_product(dev, kernel_size):
    ...
    tensor_out_ty = np.ndarray[(num_columns,), np.dtype[dtype]]
    ...
    if kernel_size == 2560:
        kernel_name = "dot_product_2560"
    elif kernel_size == 6912:
        kernel_name = "dot_product_6912"
    else:
        raise ValueError(f"Unsupported kernel size: {kernel_size}")

    # AIE Core Function declaration
    dot_kernel = Kernel(
        kernel_name, "dot.o", [tile_in_ty, tile_in_ty, tile_out_ty]
    )
